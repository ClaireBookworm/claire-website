...
function arrangeLibrary(items, width) {
  ...
  if (featured.length) {
    const cols = Math.max(2, Math.min(featured.length, Math.floor(available / 210)));
    for (let start = 0; start < featured.length; start += cols) {
      const group = featured.slice(start, start + cols);
      const baseline = shelf(285, start === 0 ? `start with these · my top ${featured.length}` : 'a few favorites');
      group.forEach((item, index) => {
        ...
        place(item, margin + available / group.length * (index + .5) - size.width / 2, baseline, size, 'face', [-1.5, 1, -.5, 1.5, -1][index % 5]);
      });
    }
  }
